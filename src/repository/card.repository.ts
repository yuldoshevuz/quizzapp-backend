...
export class CardRepository {
    ...
    async findOne(where: Prisma.CardWhereInput, answer?: boolean): Promise<Card | null> {
        return this.prismaService.card.findFirst({ select: this.select(answer), where });
    }

    async create(data: CreateCard): Promise<Card> {
        const { title, slug, authorId, categoryId, items } = data;
        return this.prismaService.card.create({ select: this.select(),
            data: {
    ...
    async updateById(cardId: string,  authorId: string, data: UpdateCard): Promise<Card> {
