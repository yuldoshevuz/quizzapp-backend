...
export class CardService {
    ...
    async create(dto: CreateCardDto, req: RequestWithUser): Promise<CardDataResponseDto> {
    ...
    async updateById(cardId: string, dto: UpdateCardDto, req: RequestWithUser): Promise<CardDataResponseDto> {
        ...
        return { card: { ...updatedCard, shareLink: this.shareLink(updatedCard.slug) } };
    }
    ...
    async getBySlug(slug: string, answer: AnswerDto): Promise<CardDataResponseDto> {
    ...
    async addItem(cardId: string, dto: CreateCardItem, req: RequestWithUser): Promise<CardDataResponseDto> {
        const existsCard = await this.getMyById(cardId, req);
        const authorId = req.user.id;

        const updatedCards = await this.cardRepository.newCardItem(dto, cardId, authorId);
