...
export class LibraryService {
  ...
  async getAll(req: RequestWithUser): Promise<LibraryResponseDto> {
    ...
    const library = await this.libraryRepository.findAll(userId, {
      card: { isPublic: true },
    });
  ...
  async createNewItem(
    ...
    const existsItem = await this.libraryRepository.findOne({
      AND: [{ cardId }, { userId }],
    });
    if (existsItem)
      throw new BadRequestException('This card is already in your library');

    const existsCard = await this.cardRepository.findOne({
      id: cardId,
      isPublic: true,
    });
