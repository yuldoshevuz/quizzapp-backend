...
import { CardItemCheckDto, CardItemTestDto, CardItemTestOptions } from "./dto/card-item.dto";
...
export class CardItemService {
    ...
    async checkAnswer(dto: CheckAnswerInputDto): Promise<CardItemCheckDto> {
        const cardItem = await this.cardItemRepository.findById(dto.cardItemId, true);
    ...
    async checkAnswerImage(dto: CheckAnswerImageDto) {
        try {
            if (!fs.existsSync(dto.file.path)) throw new ValidationException('No text image found');

            const answer = await this.ocrService.processImage(dto.file.path, dto.language);
    ...
    private makeOptions(card: Card, cardItemId: string): CardItemTestOptions[] {
        ...
        if (!correctOption) throw new NotFoundException('Card item not found in this card');
